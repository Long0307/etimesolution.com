...
<div class="content" id="content">
    <div class="box text-center" id="unset" style="background-color: white; padding-bottom: 1%;">
        ...
        <div id="digitalClock">
            ...
            <strong>
                <h1 class="hour" id="hourpc" style="margin: 0; text-align:center; font-size: 100px; color: #555;">20:22:00 PM
                </h1>
            </strong>
        ...
        <div id="analogClock" style="display: none; z-index: 1;">
            ...
            <h5 class="days" id="realDay1" style="text-align:center; font-size: 30px;"></h5>
        </div>  
        <button class="btn btn-primary my-3" type="button" id="onDigitalClock">Digital Clock</button>
        <button class="btn btn-outline-dark" type="button" id="onAnalogClock">Analog Clock</button>
    </div>
    ...
    <div class="box text-center p-3" id="tutor" style="background-color: white; padding-bottom: 1%; margin-top: 15px; padding-top: 15px; width: 100%;">
        <span style="font-size: 22px;"><strong>
            <i class="fa-solid fa-earth-americas"></i> List of city hours.</span>
        </strong>
    </div>
    ...
    <div class="row" id="boxhours">
        ...
        <div class="col-md-4 col-sm-6 col-xs-12 mt-3">
            <div class="panel text-center" style="background-color: white; height: 150px;line-height: 150px;">
                <button type="button" class="btn btn-success px-3" onclick="return addTime();">ADD</button>
            </div>
        </div>
    </div>

    <div class="box text-center p-3" id="tutor" style="background-color: white; padding-bottom: 1%; margin-top: 15px; padding-top: 15px; width: 100%;">
        <h4>Instructions for using an online watch.</h4>
        <p>- An online clock is a website that can automatically update and run according to the time frame in your location plus it can update or add time to other time frames according to the geographical location so it is easy to use. Below are instructions for using this site.</p>
        <p>Step 1: The first part you see is the time now section where the system will automatically update the time according to the time zone in your area. You can select the clock interface to see the time such as digital and analog by pressing the button as shown in the screen name. <p>
        <p>Step 2: Below is the time list of cities, you can update the city's time on the clock (by clicking on the name of the area) to see the time. Besides, in this section you can add hours to our system (by clicking the add button) so that every time you visit this page, there will be hours of that area you can see in this list.</p>
        <p>Step 3: If you want to edit the name of the region, press the pen next to the name of the time zone. You can delete the name of the time zone by clicking the trash can.</p>
        </div>
...
<div class="tableEdit">
    <div class="headertable" style="width: 100%; height: 78.39px; background-color: #0090dd; padding: 20px 15px; position: absolute; border-radius: 5px 5px 0 0;">
        <h4 style="color: white;">Add time by region</h4>
    ...
    <div class="bodyTable" style="width: 100%; height: auto; padding: 42px 15px 0px 15px; position: absolute; top: 14%; border-radius: 5px 5px 0 0;">
        <form action="" method="post" class="align-center">
            <label for="" style="padding-top: 10px;">Select region: </label>
            <select class="form-select" id="area" aria-label="Select region">
                <?php
        ...
        <label for="" style="padding-top: 10px;">City: </label>
        <select class="form-select" id="cities" aria-label="City">
            <?php
    ...
    <label for="" style="padding-top: 10px;">Title: </label>
...
<script type ="text/javascript">
    ...
    function showTimeOfAddress(id) {
        ...
        setCookie("showTimeOfAddress",1,30);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // if(getCookie("showTimeOfAddress") == 1){

                var time = new Date();

                var strDate = time.getFullYear() + "/" + (time.getMonth()+1) + "/" + time.getDate();  

                splitCurrentTime = currentTime.split(", ");

                ghep = splitCurrentTime[0]+"/"+splitCurrentTime[1];

                time = time.toLocaleString('en-US', { timeZone: ghep });

                var splitTime = time.split(" ");

                var arrayTime = splitTime[1].split(":");

                var seconds = arrayTime[2];
                var minutes = arrayTime[1];
                var hours = arrayTime[0];

                var hoursHand = document.getElementById('hour_vn'),
                minutesHand = document.getElementById('minute_vn'),
                secondsHand = document.getElementById('second_vn');

                if (hours > 12) {
                    hoursHand.style.webkitTransform = 'rotate(' + ((hours - 12) * 30 + minutes * 0.5) + 'deg)';
                    hoursHand.style.transform = 'rotate(' + ((hours - 12) * 30 + minutes * 0.5) + 'deg)';
                } else if (hours <= 12) {
                    hoursHand.style.webkitTransform = 'rotate(' + ((hours * 30) + minutes * 0.5) + 'deg)';
                    hoursHand.style.transform = 'rotate(' + ((hours * 30) + minutes * 0.5) + 'deg)';
                }

                minutesHand.style.webkitTransform = 'rotate(' + (minutes) * 6 + 'deg)';
                ...
                $("#realDay1").text(strDate);

                // }

                // ==========================================

                interval_id.push(setInterval(showTimeOfAddress(id), 10));
            }
        });
    }
